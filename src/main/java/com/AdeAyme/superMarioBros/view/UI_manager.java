...
public class UI_manager extends JPanel{
    ...
    private map_selection map_selection;

    public UI_manager(game_engine engine, int width, int height) {
        setPreferredSize(new Dimension(width, height));
        setMaximumSize(new Dimension(width, height));
        setMinimumSize(new Dimension(width, height));

        this.engine = engine;
        image_loader loader = engine.getImageLoader();

        map_selection = new map_selection();
    ...
    public void paintComponent(Graphics g){
        ...
        game_status game_status = engine.getGameStatus();
        ...
        if(game_status == game_status.START_SCREEN){
        ...
        else if(game_status == game_status.ABOUT_SCREEN){
            drawAboutScreen(g2);
        }
        else if(game_status == game_status.HELP_SCREEN){
            drawHelpScreen(g2);
        }
        else if(game_status == game_status.GAME_OVER){
            ...
            if(game_status == game_status.PAUSED){
                drawPauseScreen(g2);
            }
            else if(game_status == game_status.MISSION_PASSED){
    ...
    public String selectMapViaMouse(Point mouseLocation) {
        return map_selection.selectMap(mouseLocation);
    }

    public String selectMapViaKeyboard(int index){
        return map_selection.selectMap(index);
    }

    public int changeSelectedMap(int index, boolean up){
        return map_selection.changeSelectedMap(index, up);
    }
}